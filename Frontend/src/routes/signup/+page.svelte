<script>
  ...
  async function create(){
    ...
    if(password == password2)
    {
      ...
      if(json.status == "ok")
      {
        Cookies.set('session_id', json.data.session_id, {path: '/', domain : `${API_URL_MARKET}`, sameSite : 'None', secure:true });
        goto("/Main/Dashboard");
      }
      else
      {
          ...
          <div class="card shrink-0 w-full max-w-sm shadow-2xl bg-base-100">
            <form class="card-body">
              ...
              <div class="form-control">
                <input id="email" type="email" placeholder="Email" class="input input-bordered" required bind:value={email} on:change={validateEmail}/>
              </div>
              ...
              <div class="flex ml-2">
                ...
                <label for="hs-toggle-password-checkbox" class="text-sm text-gray-500 ms-3 dark:text-neutral-400">Show</label>
              </div>

              <div class="form-control mt-2">
                <input id="pw2" type="password" placeholder="Re-enter password" class="input input-bordered" required bind:value={password2}/>
              </div>

              <!-- Checkbox -->
              <div class="flex ml-2">
                <input on:click={showPassword2} id="hs-toggle-password-checkbox-2" type="checkbox" class="shrink-0 mt-0.5 border-gray-200 rounded text-blue-600 focus:ring-blue-500 dark:bg-neutral-800 dark:border-neutral-700 dark:checked:bg-blue-500 dark:checked:border-blue-500 dark:focus:ring-offset-gray-800">
              ...
              <div class="form-control mt-3">
                ...
                <a class="btn btn-outline btn-primary" href="/login">I already have an account</a>
              </div>
            </form>
          </div>
        </div>
