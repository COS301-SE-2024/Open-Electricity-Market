...
impl Consumer {
    pub fn update(&mut self, elapsed_time: f32, total_time: f32) {
        ...
        println!(
            "I have {} of which I'll use {}",
            results[0].units_bought, power_per_hour
        );
    ...
    pub async fn sync_grid(&self, producer_manager: Arc<Mutex<ProducerManager>>) {
        ...
        diesel::update(users)
            .filter(email.eq(&self.email))
            .set(units_bought.eq(units_bought - self.last_consumed as f64))
            .execute(connection)
            .expect("Could not update");
        println!("Used {}", self.last_consumed);

        let producer_manager = producer_manager.lock().await;
        ...
        for (_, producer) in its.iter_mut() {
            if producer.on {
                diesel::update(users)
                    .filter(email.eq(&producer.email))
                    .set(units_sold.eq(units_sold - self.last_consumed as f64))
                    .execute(connection)
                    .expect("Could not update");
