...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let consumer_manger = Arc::new(Mutex::new(ConsumerManger {
        map: HashMap::new(),
    }));

    let producer_manger = Arc::new(Mutex::new(ProducerManager {
        map: HashMap::new(),
    }));
            ...
            for user in results {
                ...
                if user.units_bought > 0.0 && !consumer_exits {
                    ...
                    let resp = client
                        ...
                        .send()
                        .await
                        .expect("Failed to connect to grid");
                    let json = resp
                ...
                let producer_exits: bool;
                match prod_manager.map.get(&user.email) {
                    None => {
                        producer_exits = false;
                    }
                    Some(_) => {
                        producer_exits = true;
                    }
